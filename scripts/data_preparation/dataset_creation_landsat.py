...
def read_get_aoi_polygon(file_path):
...
def check_tiles_overlap_landsat(bbox,date_from,date_to,data_collection=DataCollection.SENTINEL2_L2A,cloud_coverage_max=0.5,config=config_mirta):
  ...
  formatted_list = [dt.strftime('%Y-%m-%d') for dt in wfs.get_dates()]

  # print(dates)
  unique_dates = []
  for date in formatted_list:
    if formatted_list.count(date) == 1:
      unique_dates.append(date)
  return unique_dates


def tile_at_min_cc_landsat(bbox,date_from,date_to,data_collection=DataCollection.SENTINEL2_L2A,cloud_coverage_max=0.5,config=config_mirta):
  tile_dates = check_tiles_overlap_landsat(bbox,date_from,date_to,data_collection,cloud_coverage_max-0.5)

  while len(tile_dates)>0 and cloud_coverage_max >= 0:
    cloud_coverage_max = cloud_coverage_max - 0.05
    tile_dates = check_tiles_overlap_landsat(bbox,date_from,date_to,data_collection,cloud_coverage_max)

  return check_tiles_overlap_landsat(bbox,date_from,date_to,data_collection,cloud_coverage_max+0.05)